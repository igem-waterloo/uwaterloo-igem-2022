...
def full_dynamics(inital_condtions=None):
    ...
    sol = solve_ivp(reaction_ode, [0, 10], y0, args=(k, s_0, ep, dim, pyruvate), method='Radau')
    T = sol.t
    Y = sol.y
...
ax.plot(T, np.transpose(Y))
plt.title('Concentration Over Time '+ pyruvate)
plt.legend(labels=['Pyruvate', 'Acetyl CoA', 'Acetoacetyl CoA',  'HMG CoA', 'Mevalonate'
