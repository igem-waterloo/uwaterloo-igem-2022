...
def terpene_precursors(x: np.array, t: np.array) -> np.array:
  ...
  c5=900000    # Defines the width of the Gaussian peak for enzyme activity at half
              # maximum.     
                # Relevant to the following enzyme activities: MMR, MNR     
                # 
  E1=(0.139)*0.03*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))       # DXS
  E2=(0.139)*0.0225*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))    # DXR
  E3=(0.139)*0.5*np.exp(np.power(-(t-b1),2)/(2*(c1)**2))     # MCT 
  E4=(0.139)*0.0225*np.exp(np.power(-(t-b1),2)/(2*(c1)**2))     # CMK 
  E5=(0.139)*0.5*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))        # MECPS 
  E6=(0.139)*0.5*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))       # HDS  
  E7a=(0.139)*0.2*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))      # HDR (product: DMAPP)
  E7b=(0.139)*0.04*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))     # HDR (product: IPP)
  E8=(0.139)*0.3*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))       # IPPI 
  E9=(0.139)*0.1*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))       # GPPS
  E10= (0.139)*0.017*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))   # LS    
  E11= (0.365)*0.003*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))   # L3H 
  E12= (0.044)*10*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))     # IsoDH  
  E13= (0.204)*0.34*np.exp(np.power(-(t-b1), 2)/((2*c1)**2))     # IsoR  
  E14= (0.204)*0.34*np.exp(np.power(-(t-b1), 2)/((2*c1)**2))    # IsoI  
  E15= (0.365)*0.00007*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))  # MFS  
  E16a=(0.204)*0.0015*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))  # PR  (product: (-)-menthone)
  E16b=(0.204)*0.00015*np.exp(np.power(-(t-b1), 2)/(2*(c1)**2))  # PR  (product: (+)-isomenthone)
  E17a=(0.204)*0.0011*np.exp(np.power(-(t-b5), 2)/(2*(c5)**2))  # MMR (product: (-)-menthol)
  E17b=(0.204)*0.0011*np.exp(np.power(-(t-b5), 2)/(2*(c5)**2))  # MMR (product: (+)-neoisomenthol)
  E18a=(0.204)*0.00001*np.exp(np.power(-(t-b5), 2)/(2*(c5)**2))  # MNR (product: (+)-neomenthol)
  E18b=(0.204)*0.00001*np.exp(np.power(-(t-b5), 2)/(2*(c5)**2)) # MNR (product: (+)-isomenthol)
  ...
  GN = 1+ 1/(1+c*np.exp(-k*t)) # at day 15, gland number is 86.7 # of total gland number at day 30
  ...
  if t< 1296000:  # (patterns of enzymes from 0 to 15 days after leaf initiation)   
    ...
    #                   half maximum.    
      ...
      E12=(0.044)*1*np.exp(np.power(-(t-b4), 2)/(2*(c4)**2))             # IsoDH    
      E13=(0.204)*0.0044*np.exp(np.power(-(t-b2), 2)/(2*(c2)**2))        # IsoR
      E14=(0.204)*0.0044*np.exp(np.power(-(t-b2), 2)/(2*(c2)**2))        # IsoI
      E16a=(0.204)*0.00014*np.exp(np.power(-(t-b2), 2)/(2*(c2)**2))      # PR (product: (-)-menthone)
      E16b=(0.204)*0.000014*np.exp(np.power(-(t-b2), 2)/(2*(c2)**2))     # PR (product: (+)-isomenthone)
